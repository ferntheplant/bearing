...
- **`bearing ls` filters by type, readiness, blocked-ness, project, or a query, and combining filters
  intersects rather than erroring.** A `--project` that no map carries is an error naming the maps that exist.
