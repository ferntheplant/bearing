...
| 6   | [Closing](./06-closing.md)                                 | Designed  |
...
invocation, minting an id against the ids already on disk. `bearing ls` filtering and the frontier's
blocking-graph input are built. Derived map, fog, trail, and integrity analysis is still to come.
