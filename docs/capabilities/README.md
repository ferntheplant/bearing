...
and strip the closed id from every blocker list as a lossless rewrite. `bearing check` reads the whole tracker
and reports every parse failure, the five integrity error classes, and the single warning in one run, with the
error/warning distinction carried in the `--json` output and the exit status. The design-ticket dry run and
closing a map are still to come.
